...
public class StudentDao {
  ...
  public boolean addStudent(Student addingStudent) {                 // Function for adding student.
    ...
    try {
        PreparedStatement ps = con.prepareStatement(sql);     // Hey database connection, take this SQL query, prepare it,and give me a   
                ...
                ps.setString(1, addingStudent.getName()); // Put the student's name into the first'?'placeholder of the SQL query.
  ...
  public List<Student> getAllStudents() {        // Function for show student's data, will return a List containing Student objects.
    ...
    try {
      ...
      while (rs.next()) {
        ...
        Student student = new Student(id, name, age, email, gender, dateOfBirth, course); // Database rows are not Java objects.That's why we created. and Inside the loop,
                                                                                           // every new student replaces the previous reference.so with list we save every objects.
...
public void updateStudent(Student updatingStudent) {         //Function for Update student's data. 
...
public void deleteStudent(int id) {
    ...
    try {
        PreparedStatement ps = con.prepareStatement(sql);
             ...
             ps.setInt(1, id);      // it means: 1 → first ? and id → the value to put into that ?
